...
from services.orders_store import list_orders, list_orders_for_seller
...
def _order_date_kst(created_at: str | None) -> str:
    """UTC ISO created_at → KST 날짜(YYYY-MM-DD)."""
...
def dashboard(user: dict = Depends(get_current_user)):
    ...
    for o in paid_orders:
        created = _order_date_kst(o.get("created_at"))
    ...
    today_order_count = sum(
        1 for o in orders if _order_date_kst(o.get("created_at")) == today_str
    )
    pending_count = sum(
        1
        for o in orders
        if o.get("payment_status") == "paid"
        and (o.get("fulfillment_status") or "pending")
        in ("pending", "preparing", "shipping")
    )
